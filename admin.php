    ...
    <div class="container">
        ...
        <table class="table table-stripted table-bordered">
            ...
            <?php foreach ($all as $user) : ?>
                <tr>
                    <td><?= $security::h($user->id) ?></td>
                    <td><?= $security::h($user->name) ?></td>
                    <td><?= $security::h($user->email) ?></td>
                    <td><?= $security::H($user->phone) ?></td>
                    <td>
                        <?php if ($user->role === '1') : ?>
                        ...
                        <?php if ($auth->value > 1) : ?>
                            <div class="btn-group dropdown">
                                ...
                                <?php if ($user->id !== $auth->id) : ?>
                                    <a href="_actions/delete.php?id=<?= $user->id ?>&csrf=<?= $token ?>" class="btn btn-sm btn-outline-danger" onClick="return confirm('Are you sure?')">Delete</a>
                                <?php endif ?>
                            </div>
                        <?php else : ?>
